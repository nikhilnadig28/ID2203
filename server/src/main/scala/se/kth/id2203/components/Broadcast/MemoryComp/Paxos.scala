...
import se.kth.id2203.components.Broadcast.Consensus
import se.kth.id2203.components.Broadcast.PaxosComponents._
...
class Paxos(paxosInit: Init[Paxos]) extends ComponentDefinition {
  ...
  val (self, rank, numProcesses) = paxosInit match {
    case Init(s: NetAddress, qSize: Int) => (s, toRank(s), qSize)
  }
